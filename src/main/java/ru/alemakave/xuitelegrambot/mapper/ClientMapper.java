...
import ru.alemakave.xuitelegrambot.dto.ClientAddDtoSettings;
import ru.alemakave.xuitelegrambot.model.Client;

import java.util.List;

@Mapper(componentModel = "spring", imports = {List.class, ClientAddDtoSettings.class, ObjectMapper.class})
public abstract class ClientMapper {
    @Mapping(target = "id", expression = "java(connectionId)")
    @Mapping(target = "settings", expression = "java(new ObjectMapper().writeValueAsString(new ClientAddDtoSettings(List.of(client))))")
